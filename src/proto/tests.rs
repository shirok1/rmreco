...
fn graphic_data_size() {
    ...
    for i in 0b000..=0b111 {
        ...
        let ((_, rest_byte_size), parsed) = graphic::GraphicData::from_bytes((&data[..], 0)).unwrap();
        assert_eq!(rest_byte_size, 0);
        let bytes: Vec<u8> = parsed.try_into().unwrap();
...
fn referee_warning_size() {
    ...
    for i in 0b01..=0b11 {
        data[0] = i;
        let ((_, rest_byte_size), parsed) = RefereeWarning::from_bytes((&data[..], 0)).unwrap();
        assert_eq!(rest_byte_size, 0);
        let bytes: Vec<u8> = parsed.try_into().unwrap();
        assert_eq!(bytes.as_slice(), data);
    }
...
fn frame_parse() {
    let data: [u8; 0x19] = [
        0xA5, 0x10, 0x00, 0x6C, 0x4F, 0x03, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x82, 0x08
    ];
    let ((_, rest_byte_size), parsed) = Frame2::from_bytes((&data[..], 0)).unwrap();
    assert_eq!(rest_byte_size, 0);
}
